		...
		if (count>t.length) {
			...
			for (int i=0;i<t.length&&!cont;i++) {
				...
				if (search(t,ref,count)==-1) {
					...
					type=sc.next();
					if (!type.equalsIgnoreCase("Cercanias")&&!type.equalsIgnoreCase("Ave")&&!type.equalsIgnoreCase("Mercancias")) {
						type="Indefinido";
						System.out.println("El tipo introducido no coincide con los mostrados. Por defecto, el tipo del tren queda indefinido.");
						do {
